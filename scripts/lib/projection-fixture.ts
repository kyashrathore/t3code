...
function canonicalPathThroughExistingAncestor(target: string): string {
  ...
  const canonicalExisting = NodeFS.realpathSync.native(existing);
  return NodePath.join(canonicalExisting, NodePath.relative(existing, resolved));
}
...
export function assertDisposableProjectionTarget(input: {
  ...
  const canonicalDbPath = canonicalPathThroughExistingAncestor(input.dbPath);
  const canonicalDisposableRoot = canonicalPathThroughExistingAncestor(input.disposableRoot);
  if (!isWithin(canonicalDisposableRoot, canonicalDbPath)) {
    throw new Error(
  ...
  for (const liveHome of [
    ...
    if (liveHome && isWithin(canonicalPathThroughExistingAncestor(liveHome), canonicalDbPath)) {
      throw new Error(`Refusing to write a projection fixture inside live T3 home ${liveHome}.`);
    }
  }
}

function assertDatabaseTargetsPath(database: NodeSqlite.DatabaseSync, dbPath: string): void {
  const main = (
    database.prepare("PRAGMA database_list").all() as unknown as ReadonlyArray<{
      readonly name: string;
  ...
  if (
    !main?.file ||
    canonicalPathThroughExistingAncestor(main.file) !== canonicalPathThroughExistingAncestor(dbPath)
  ) {
    throw new Error("Projection fixture database handle does not target dbPath.");
  }
}
...
export function writeProjectionFixture(input: {
  ...
  try {
    assertDatabaseTargetsPath(database, input.dbPath);
