...
function extractPairingUrl(stdout: string): string | null {
  const labeled = stdout.match(/Pairing URL:\s+(\S+)/);
  if (labeled?.[1]) {
    return labeled[1];
  }
  const structured = stdout.match(/pairingUrl:\s+(\S+)/);
  if (structured?.[1]) {
    return structured[1].replace(/[.,]$/, "");
  }
  const tokenUrl = stdout.match(/https?:\/\/\S+\/pair#token=[^\s]+/);
  return tokenUrl?.[0]?.replace(/[.,]$/, "") ?? null;
}

async function waitForPairingUrl(getStdout: () => string, process: ChildProcess): Promise<string> {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    const pairingUrl = extractPairingUrl(getStdout());
    if (pairingUrl) {
      return pairingUrl;
    }
  ...
  throw new Error(`Timed out waiting for perf server pairing URL.\n${getStdout()}`);
}
...
async function verifyBuiltArtifacts(): Promise<void> {
  await Promise.all([access(serverBinPath), access(serverClientIndexPath)]).catch(() => {
    throw new Error(
      `Built perf artifacts are missing. Expected ${serverBinPath} and ${serverClientIndexPath}. Run vp run --filter t3 build first.`,
    );
  });
}
...
export async function startPerfAppHarness(
  ...
  const seededState = await (async () => {
    const seedProcess = spawn(
      process.execPath,
      ["apps/server/scripts/seedPerfState.ts", options.seedScenarioId],
      {
        cwd: repoRoot,
        env: process.env,
  ...
  try {
    const pairingUrl = await waitForPairingUrl(() => stdoutBuffer, serverProcess);
    ...
    try {
      await page.waitForFunction(() => window.location.pathname !== "/pair", {
        timeout: 20_000,
      });
    } catch {
      const bodyText = await page.locator("body").innerText();
      await page.screenshot({ path: join(artifactDir, "pairing-timeout.png") }).catch(() => {});
      throw new Error(
        `Pairing did not complete. url=${page.url()}\nconsole=${consoleErrors.join(" | ")}\nbody=${bodyText.slice(0, 2500)}`,
      );
    }

    try {
      await page.getByText("All projects", { exact: true }).first().waitFor({ timeout: 45_000 });
    } catch (error) {
      const bodyText = await page.locator("body").innerText();
      await page.screenshot({ path: join(artifactDir, "sidebar-timeout.png") }).catch(() => {});
      throw new Error(
        `Perf app did not finish loading the sidebar. url=${page.url()} title=${firstProjectTitle}\nconsole=${consoleErrors.join(" | ")}\nbody=${bodyText.slice(0, 2500)}\n${error instanceof Error ? error.message : String(error)}`,
      );
    }
